...
def main():
    ...
    parser.add_argument("subcommand", choices=['posthit', 'getresults', 'reformat',
        "preparefiles", "anonymize", "postbatches"],
...
def getresults(label, batch=False):
    results = os.popen("""
        HERE=`pwd`
        cd $MTURK_CMD_HOME/bin
...
def postbatches(label):
    config_file = label + ".config"
    settings = open(config_file, 'r')
    json_string = settings.read()
    settings.close()
    config_dict = json.loads(re.sub("\n", "", json_string))
    total = int(config_dict['totalassignments'])
    per = int(config_dict['numberofassignments'])
    remainder = total%per
    if remainder != 0:
        n_batches = (total/per)+1
    else:
        n_batches = total/per
    i = 0
    while i < n_batches:
        ...
        properties.write(
            ...
            "\nassignments: " + per + \
            "\nannotation: ${condition}" + \
            "\nassignmentduration:" + config_dict["assignmentduration"] + \
            "\nhitlifetime:" + config_dict["hitlifetime"] + \
            "\nautoapprovaldelay:" + config_dict["autoapprovaldelay"]
            )
        if (config_dict["USonly?"] == "y" or \
                config_dict["USonly?"] == "Y" or \
                config_dict["USonly?"] == "yes" or \
                config_dict["USonly?"] == "Yes" or \
                config_dict["USonly?"] == "true" or \
                config_dict["USonly?"] == "True" or \
                config_dict["USonly?"] == "T" or \
                config_dict["USonly?"] == "1"):
            properties.write(
                "\nqualification.1:00000000000000000071"+\
                "\nqualification.comparator.1:EqualTo"+\
                "\nqualification.locale.1:US"+\
        ...
        while getResults(label,True)[0] < per:
            time.sleep(300)
        i+=1
